...
export const getDashboardStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const recentIncidentsRaw = await IncidentReport.find()
      .sort({ createdAt: -1 })
      .limit(5)
      .select('referenceId createdAt violenceType status locationText areaCouncil _id') // MODIFIED: Selected areaCouncil instead of locationText
      .lean();
